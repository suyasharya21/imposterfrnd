...
function Laser({ start, end, color, timestamp }: { start: [number, number, number], end: [number, number, number], color: string, timestamp: number }) {
  ...
  useFrame(() => {
    ...
    const opacity = Math.max(0, 1 - age / 1000); // Fade out over 1s

    if (ref.current) {
      const mat = ref.current.material as THREE.MeshBasicMaterial;
      mat.opacity = opacity;
      // Shrink width as it fades
      ref.current.scale.x = ref.current.scale.y = Math.pow(opacity, 0.5);
    }
    if (glowRef.current) {
      const mat = glowRef.current.material as THREE.MeshBasicMaterial;
      mat.opacity = opacity * 0.4;
      glowRef.current.scale.x = glowRef.current.scale.y = Math.pow(opacity, 0.3) * 2;
    }
  });
